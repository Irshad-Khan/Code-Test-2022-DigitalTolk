...
trait NotificationTrait
{
     ...
     * @param $duration
     */
    public function sendNotificationChangePendingStatus($user, $job, $language, $due, $duration)
    {
        $data = array();
     ...
     * @param $job_id
     */
    public function sendNotificationsByAdminCancelJob($job_id)
    {
     ...
     * @param $old_lang
     */
    public function sendChangedLanguageNotification($job, $old_lang)
    {
        $user = $job->user()->first();
     ...
     */
    public function sendChangeTranslatorNotification($job, $current_translator, $new_translator)
    {
        $user = $job->user()->first();
        if (!empty($job->user_email)) {
            $email = $job->user_email;
     ...
     * @param $old_time
     */
    public function sendChangeDateNotification($job, $old_time)
    {
        $user = $job->user()->first();
        if (!empty($job->user_email)) {
            $email = $job->user_email;
